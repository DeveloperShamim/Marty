...
<div class="space-y-4 sm:space-y-6 max-w-full">
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-gray-200 pb-4">
    ...
    <div class="bg-white border border-slate-200 px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-xl shadow-2xs flex items-center gap-3 shrink-0 self-start md:self-auto w-full sm:w-auto">
      <div class="h-9 w-9 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center text-sm font-bold border border-emerald-100 shrink-0">
        ⚡
      </div>
      <div>
        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Active Right Now</span>
        <div class="flex items-baseline gap-1.5">
          <strong class="text-base sm:text-lg font-black font-mono text-slate-900">{{ number_format($activeRealtimeCount) }}</strong>
  ...
  <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-3.5 sm:p-4 shadow-2xs hover:border-slate-300 transition">
      ...
      <p class="text-[10px] sm:text-[11px] text-slate-400 mt-1">Unique 7-day IPs</p>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-3.5 sm:p-4 shadow-2xs hover:border-slate-300 transition col-span-2 sm:col-span-1">
      ...
      <p class="text-[10px] sm:text-[11px] text-slate-400 mt-1">Total tracked logs</p>
    </div>
  ...
  <div class="card p-4 sm:p-6 space-y-4 sm:space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-slate-100 pb-4">
      <div>
        <div class="flex items-center gap-2 flex-wrap">
          <h3 class="font-extrabold text-sm sm:text-base text-slate-900 flex items-center gap-2">
            <span>📈</span> 14-Day Traffic Line Chart
          </h3>
          <span class="px-2 py-0.5 text-[10px] sm:text-[11px] font-extrabold rounded-full bg-brand-50 text-brand-700 border border-brand-200">
            Interactive Line Chart
          </span>
        </div>
        <p class="text-xs text-slate-500 mt-0.5">Daily unique visitor trajectory and trend line</p>
      </div>

      <div class="flex items-center gap-3 text-xs font-bold shrink-0 flex-wrap">
        <div class="flex items-center gap-1.5 text-slate-600">
          <span class="w-2.5 h-2.5 rounded-full bg-brand-600"></span>
          <span>Unique Visitors</span>
        </div>
        <span class="text-slate-700 font-extrabold bg-slate-100 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-xl border border-slate-200 text-xs">
  ...
  <div class="card overflow-hidden">
    <div class="p-3.5 sm:p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div>
        ...
        <p class="text-[11px] sm:text-xs text-slate-500">Real-time IP logs captured by storefront middleware</p>
      </div>
    ...
    <div class="block sm:hidden divide-y divide-slate-100 bg-white">
      @forelse($logs as $log)
        @php
          $isMobile = str_contains(strtolower($log->user_agent), 'mobile') || str_contains(strtolower($log->user_agent), 'android') || str_contains(strtolower($log->user_agent), 'iphone');
        @endphp
        <div class="p-3.5 space-y-2">
          <div class="flex items-center justify-between gap-2">
            <span class="px-2.5 py-1 rounded-md bg-slate-100 text-slate-900 border border-slate-200 font-mono font-bold text-xs">
              {{ $log->ip_address }}
            </span>
          ...
          <div class="flex items-center justify-between gap-2 text-xs">
            <div class="flex items-center gap-1.5">
              @if($isMobile)
                <span class="px-2 py-0.5 text-[10px] font-extrabold rounded bg-amber-50 text-amber-800 border border-amber-200">Mobile</span>
              @else
                <span class="px-2 py-0.5 text-[10px] font-extrabold rounded bg-slate-100 text-slate-700 border border-slate-200">Desktop</span>
              @endif
              <span class="text-slate-600 font-bold">{{ \Illuminate\Support\Carbon::parse($log->visit_date)->format('d M Y') }}</span>
            </div>
          </div>

          @if($log->user_agent)
            <p class="text-[11px] text-slate-500 font-mono truncate bg-slate-50 p-2 rounded-lg border border-slate-100" title="{{ $log->user_agent }}">
              {{ $log->user_agent }}
            </p>
          @endif
        </div>
