...
<body class="bg-[#F5F8F7] min-h-screen flex items-center justify-center p-4 text-ink">
  <div class="w-full max-w-md">
    ...
    <div class="bg-white rounded-xl border border-stone-200 shadow-sm p-8">
      ...
      @if(testing_mode())
          ...
          <div class="border border-t-0 px-3 py-3 text-sm text-slate-800" style="border-color: color-mix(in srgb, #0f766e 35%, #e5e7eb); background: color-mix(in srgb, #0f766e 8%, #ffffff);">
            ...
            <p class="mt-2"><span class="text-slate-500">Email:</span> <code class="font-mono font-medium px-1 rounded" style="background: color-mix(in srgb, #0f766e 12%, #ffffff);">{{ config('app.testing_admin_email') }}</code></p>
            <p class="mt-1"><span class="text-slate-500">Password:</span> <code class="font-mono font-medium px-1 rounded" style="background: color-mix(in srgb, #0f766e 12%, #ffffff);">{{ config('app.testing_admin_password') }}</code></p>
    ...
    <p class="text-center text-xs text-stone-400 mt-6">© {{ date('Y') }} {{ site_name() }}. Admin Panel.</p>
    <p class="text-center mt-2"><a href="{{ route('home') }}" class="text-sm text-stone-500 hover:text-brand-600">← Back to store</a></p>
  </div>
</body>
</html>
