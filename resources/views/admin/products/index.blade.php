...
<div class="space-y-6">
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h2 class="text-xl font-extrabold text-stone-900">Products</h2>
      <p class="text-xs text-stone-500 mt-0.5">Manage store catalog, stock quantities, and bulk operations.</p>
    </div>

    <div class="flex items-center gap-2 flex-wrap">
      {{-- Export CSV Button --}}
      <a href="{{ route('admin.products.export', request()->only(['q', 'category'])) }}" class="px-3.5 py-2 text-xs font-bold text-stone-700 bg-white border border-stone-300 hover:bg-stone-50 rounded-xl transition-colors inline-flex items-center gap-1.5 shadow-xs" title="Export matching products to CSV">
        <svg class="w-4 h-4 text-stone-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
        <span>Export CSV</span>
      </a>

      {{-- Import CSV Button --}}
      <button type="button" onclick="document.getElementById('importProductModal').classList.remove('hidden')" class="px-3.5 py-2 text-xs font-bold text-stone-700 bg-white border border-stone-300 hover:bg-stone-50 rounded-xl transition-colors inline-flex items-center gap-1.5 shadow-xs">
      ...
      {{-- New Product Button --}}
      <a href="{{ route('admin.products.create') }}" class="btn-primary flex items-center gap-1">
  ...
  <div class="card bg-white rounded-2xl border border-stone-200 shadow-xs overflow-hidden">
    <form method="GET" class="p-4 border-b border-stone-100 flex flex-wrap gap-3 items-center bg-stone-50/50">
      <input type="text" name="q" value="{{ $q }}" placeholder="Search by product name, SKU, or variant SKU (e.g. Black/M, UNI-656708-BlackM)..." class="flex-1 min-w-[260px] border border-stone-300 rounded-xl px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white" />
      <select name="category" onchange="this.form.submit()" class="border border-stone-300 rounded-xl px-3.5 py-2 text-sm bg-white font-medium">
        <option value="">All Categories</option>
        @foreach($categories as $cat)
          <option value="{{ $cat->id }}" @selected((string) $category === (string) $cat->id)>{{ $cat->name }}</option>
        @endforeach
      </select>
      <button class="btn-primary text-xs py-2 px-4">Filter</button>
      @if(!empty($q) || !empty($category))
        <a href="{{ route('admin.products.index') }}" class="text-xs text-stone-500 hover:text-stone-800 underline">Reset</a>
      @endif
    </form>
...
<div id="bulkActionsBar" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-slate-900 text-white rounded-2xl shadow-2xl px-6 py-3.5 flex items-center gap-5 hidden transition-all duration-200 border border-slate-700">
  <div class="flex items-center gap-2">
    <span class="h-2 w-2 rounded-full bg-emerald-400 animate-pulse"></span>
    <span class="text-xs font-extrabold text-slate-100 tracking-wide"><span id="selectedCount">0</span> products selected</span>
  </div>
  <form id="bulkDeleteForm" action="{{ route('admin.products.bulk-delete') }}" method="POST" onsubmit="return confirm('Are you sure you want to delete all selected products? This action cannot be undone.')">
    ...
    <button type="submit" class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-extrabold text-xs rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer hover:scale-105 active:scale-95">
  ...
  <div class="bg-white rounded-2xl shadow-xl border border-stone-200 w-full max-w-lg overflow-hidden">
    <div class="p-6 border-b border-stone-100 flex items-center justify-between">
      <div class="flex items-center gap-2.5">
        ...
        <h3 class="font-extrabold text-stone-900 text-base">Bulk Import Products (CSV)</h3>
      </div>
    ...
    <form action="{{ route('admin.products.import') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
      ...
      <div class="bg-amber-50 border border-amber-200 text-amber-900 text-xs rounded-xl p-4 space-y-2">
      ...
      <div class="space-y-2">
        ...
        <input type="file" id="csv_file" name="csv_file" accept=".csv, text/csv" required class="w-full text-xs font-medium text-stone-600 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-brand-50 file:text-brand-700 hover:file:bg-brand-100 border border-stone-300 rounded-xl bg-stone-50 cursor-pointer" />
      </div>
      ...
      <div class="pt-2 flex items-center justify-end gap-3 border-t border-stone-100">
        ...
        <button type="submit" class="px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white text-xs font-bold rounded-xl shadow-xs transition-colors inline-flex items-center gap-1.5">
