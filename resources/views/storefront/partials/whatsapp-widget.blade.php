...
<aside id="chatSpeedDialRoot" aria-label="Customer Support Chat" class="fixed {{ request()->routeIs('checkout.*') ? 'bottom-24 right-4 sm:right-6' : (request()->routeIs('product.show') ? 'bottom-28 right-4 sm:right-6' : 'bottom-20 right-5 sm:right-6') }} z-50 select-none font-sans flex flex-col items-end gap-2.5">
  ...
  <button type="button" id="chatSpeedDialToggleBtn" class="group relative flex items-center justify-center w-12 h-12 bg-emerald-500 hover:bg-emerald-600 active:scale-95 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer" aria-label="Toggle Chat Options" aria-expanded="false">
    
    <!-- Pulse ring (visible when closed) -->
    <span id="chatSpeedDialPulseRing" class="absolute -inset-0.5 rounded-full bg-emerald-400 opacity-40 animate-ping pointer-events-none"></span>

    <!-- WhatsApp & Chat Icon (Closed state) -->
    <div id="chatSpeedDialIconNormal" class="relative z-10 transition-all duration-300 transform group-hover:scale-110">
