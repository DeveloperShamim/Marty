    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-3 sm:gap-6">
      ...
      <form action="{{ route('shop') }}" method="GET" class="hidden md:flex flex-1 max-w-xl lg:max-w-2xl mx-auto">
        <div class="relative flex items-center w-full h-11 sm:h-12 rounded-full border border-emerald-200/90 bg-emerald-50/20 hover:bg-white focus-within:bg-white focus-within:border-emerald-600 focus-within:ring-4 focus-within:ring-emerald-600/10 transition-all duration-200 shadow-2xs overflow-hidden px-1.5">
          ...
          <input type="search" name="q" value="{{ request('q') }}" placeholder="{{ setting('search_placeholder', 'Search raw honey, mustard oil, deshi ghee, chia seeds...') }}" class="flex-1 px-2.5 text-xs sm:text-sm font-semibold bg-transparent focus:outline-none text-stone-800 placeholder:text-stone-400" autocomplete="off" />
          <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs px-5 h-8.5 rounded-full flex items-center justify-center transition-colors shadow-2xs shrink-0 cursor-pointer" aria-label="Search">
            <span>Search</span>
          </button>
        </div>
      </form>
