...
@if($promoText !== '')
  <div class="bg-ink text-white text-xs sm:text-sm text-center py-2 px-4">
    @if($promoLink !== '')
      <a href="{{ $promoLink }}" class="hover:text-brand-300">{!! strip_tags($promoText, '<b><strong><span>') !!}</a>
    @else
...
<header class="site-header sticky top-0 z-40">
  <div class="bg-white border-b border-stone-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-5 py-3 flex items-center gap-3 sm:gap-4">
      ...
      <form action="{{ route('shop') }}" method="GET" class="hidden md:flex flex-1 max-w-xl mx-auto">
        <div class="flex w-full rounded-md border border-stone-200 overflow-hidden bg-white focus-within:ring-2 focus-within:ring-brand-600/25">
          <input type="search" name="q" value="{{ request('q') }}" placeholder="{{ setting('search_placeholder', 'Search Products...') }}" class="flex-1 px-3 py-2.5 text-sm focus:outline-none text-ink" autocomplete="off" />
      ...
      <div class="ml-auto flex items-center gap-1 sm:gap-3 shrink-0">
        ...
        <button type="button" data-open-cart class="relative flex items-center gap-1.5 p-2 text-ink hover:text-brand-600" aria-label="Cart">
          ...
          <span data-cart-count class="cart-count absolute -top-0.5 -right-0.5 h-4 min-w-4 px-1 rounded-full bg-brand-600 text-white text-[10px] font-bold flex items-center justify-center {{ $cartCount ? '' : 'hidden' }}">{{ $cartCount }}</span>
        </button>
      </div>
    </div>
  </div>
  ...
  <div class="hidden lg:block bg-white border-b border-stone-200/80 text-stone-700 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-5 flex items-center justify-between gap-2 h-11 text-sm font-semibold">
      <nav class="flex flex-1 items-center justify-between gap-1 py-1">
        <a href="{{ route('home') }}" class="px-2.5 py-1.5 whitespace-nowrap rounded-lg transition-colors {{ request()->routeIs('home') ? 'bg-brand-50 text-brand-600 font-bold' : 'text-stone-700 hover:text-brand-600 hover:bg-stone-50' }}">Home</a>
        <a href="{{ route('shop') }}" class="px-2.5 py-1.5 whitespace-nowrap rounded-lg transition-colors {{ request()->routeIs('shop*') && ! request('flash') ? 'bg-brand-50 text-brand-600 font-bold' : 'text-stone-700 hover:text-brand-600 hover:bg-stone-50' }}">Shop</a>
        @if($hasFlashSale ?? false)
          <a href="{{ route('shop', ['flash' => 1]) }}" class="px-2.5 py-1.5 whitespace-nowrap rounded-lg transition-colors {{ request('flash') ? 'bg-brand-50 text-brand-600 font-bold' : 'text-stone-700 hover:text-brand-600 hover:bg-stone-50' }}">Deals &amp; Offers</a>
        @endif
        @foreach($topCats as $cat)
          <a href="{{ route('shop.category', $cat) }}" class="px-2.5 py-1.5 whitespace-nowrap rounded-lg transition-colors {{ optional($activeCategory ?? null)->id === $cat->id ? 'bg-brand-50 text-brand-600 font-bold' : 'text-stone-700 hover:text-brand-600 hover:bg-stone-50' }}">{{ $cat->name }}</a>
        @endforeach
        ...
        @endif
        <a href="{{ route('contact') }}" class="px-2.5 py-1.5 whitespace-nowrap rounded-lg transition-colors {{ request()->routeIs('contact') ? 'bg-brand-50 text-brand-600 font-bold' : 'text-stone-700 hover:text-brand-600 hover:bg-stone-50' }}">Help &amp; Support</a>
      </nav>
      <a href="{{ route('track') }}" class="ml-4 shrink-0 bg-brand-600 hover:bg-brand-700 text-white text-xs sm:text-sm font-bold px-3.5 sm:px-4 py-1.5 rounded-lg shadow-sm transition">TRACK ORDER</a>
    </div>
  </div>
