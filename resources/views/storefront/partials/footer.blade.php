...
<footer class="bg-slate-900 text-slate-100 mt-14 border-t border-slate-800">
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-5 py-12 grid grid-cols-2 md:grid-cols-4 gap-8 text-sm">
    <div class="col-span-2 md:col-span-1">
      <p class="text-2xl font-extrabold mb-2 text-white tracking-tight">{{ $site }}</p>
      @if($footerText !== '')
        <p class="text-slate-400 leading-relaxed text-sm">{{ $footerText }}</p>
      @endif
      @if($phone !== '')
        <a href="tel:{{ preg_replace('/\s+/', '', $phone) }}" class="inline-flex items-center gap-2 mt-4 bg-brand-500 hover:bg-brand-600 text-white font-bold px-4 py-2.5 rounded-xl shadow-md transition-all">
      ...
      @if($facebook || $instagram || $twitter)
        <div class="flex gap-2 mt-4">
          @if($facebook)
            <a href="{{ $facebook }}" target="_blank" rel="noopener" class="h-9 w-9 rounded-xl bg-slate-800 hover:bg-brand-500 border border-slate-700/60 text-slate-300 hover:text-white flex items-center justify-center transition-all" aria-label="Facebook">
          ...
          @endif
          @if($instagram)
            <a href="{{ $instagram }}" target="_blank" rel="noopener" class="h-9 w-9 rounded-xl bg-slate-800 hover:bg-brand-500 border border-slate-700/60 text-slate-300 hover:text-white flex items-center justify-center transition-all" aria-label="Instagram">
          ...
          @endif
          @if($twitter)
            <a href="{{ $twitter }}" target="_blank" rel="noopener" class="h-9 w-9 rounded-xl bg-slate-800 hover:bg-brand-500 border border-slate-700/60 text-slate-300 hover:text-white flex items-center justify-center transition-all" aria-label="X (Twitter)">
    ...
    <div>
      ...
      <ul class="space-y-2.5 text-slate-300">
        ...
        <li><a href="{{ route('shop') }}" class="hover:text-brand-400 transition-colors">Shop</a></li>
        @if($hasFlashSale ?? false)
          <li><a href="{{ route('shop', ['flash' => 1]) }}" class="hover:text-brand-400 transition-colors">Deals &amp; Offers</a></li>
        @endif
    ...
    <div>
      ...
      <ul class="space-y-2.5 text-slate-300">
        <li><a href="{{ route('contact') }}" class="hover:text-brand-400 transition-colors">Contact Us</a></li>
        <li><a href="{{ route('track') }}" class="hover:text-brand-400 transition-colors">Track Order</a></li>
        <li><a href="{{ route('login') }}" class="hover:text-brand-400 transition-colors">My Account</a></li>
        <li><a href="{{ route('terms') }}" class="hover:text-brand-400 transition-colors">Terms of Service</a></li>
        <li><a href="{{ route('privacy') }}" class="hover:text-brand-400 transition-colors">Privacy Policy</a></li>
      </ul>
    </div>
