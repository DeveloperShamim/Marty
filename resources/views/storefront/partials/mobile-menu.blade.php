<aside id="mobileMenu" class="mobile-menu fixed inset-y-0 left-0 z-50 flex w-80 max-w-[85%] -translate-x-full flex-col bg-white shadow-2xl lg:hidden transition-transform duration-300">
  ...
  <div class="border-b border-stone-100 bg-brand-50/50 p-5">
    @auth
      ...
      <div class="mt-4 grid grid-cols-2 gap-2">
        <a href="{{ route('account') }}" class="rounded-xl bg-white px-3 py-2.5 text-center text-sm font-semibold text-ink ring-1 ring-stone-200 hover:bg-brand-50">My Account</a>
    ...
    @else
      ...
      <div class="mt-4 grid grid-cols-2 gap-2">
        <a href="{{ route('login') }}" class="rounded-xl bg-brand-600 px-3 py-2.5 text-center text-sm font-semibold text-white hover:bg-brand-700">Sign in</a>
        <a href="{{ route('register') }}" class="rounded-xl bg-white px-3 py-2.5 text-center text-sm font-semibold text-ink ring-1 ring-stone-200 hover:bg-brand-50">Register</a>
      </div>
    @endauth
  </div>

  <nav class="flex-1 overflow-y-auto px-4 py-3 text-sm font-medium">
    <a href="{{ route('home') }}" class="block py-2.5 hover:text-brand-700">🏠 Home</a>
    <a href="{{ route('shop') }}" class="block py-2.5 hover:text-brand-700">🌿 Shop All Products</a>
    ...
    @endif
    <a href="{{ route('contact') }}" class="block py-2.5 hover:text-brand-700">Help &amp; Support</a>
    <a href="{{ route('login') }}" class="block py-2.5 hover:text-brand-700 sm:hidden">Account</a>

    @if(isset($navCategories) && $navCategories->isNotEmpty())
      <div class="mt-4 pt-3 border-t border-stone-100">
        <p class="mb-2 text-[11px] font-extrabold uppercase tracking-wider text-brand-700">Organic Categories</p>
        <div class="space-y-1">
          @foreach($navCategories as $cat)
            <a href="{{ route('shop.category', $cat) }}" class="flex items-center justify-between py-2 px-2 hover:bg-brand-50 hover:text-brand-700 rounded-lg transition-colors">
    ...
    <div class="mt-6 pt-4 border-t border-stone-100">
      <a href="{{ route('track') }}" class="flex items-center justify-center gap-2 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold py-3 px-4 text-sm shadow-sm transition">
