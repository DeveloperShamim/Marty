...
@section('content')
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-5">
    ...
    {{-- 8. VERIFIED CUSTOMER REVIEWS & SOCIAL PROOF (Hidden on mobile) --}}
    @if($homeReviews->isNotEmpty())
      <section class="hidden md:block mt-14 sm:mt-16 mb-12" data-reveal>
        <div class="text-center max-w-xl mx-auto mb-8 sm:mb-10 px-4">
          ...
          @foreach($homeReviews as $review)
            <div class="relative overflow-hidden rounded-2xl border border-stone-200/80 bg-white p-5 sm:p-6 shadow-2xs hover:shadow-md transition-all flex flex-col justify-between group">
              ...
              <div class="mt-4 pt-4 border-t border-stone-100 flex items-center justify-between gap-2">
                ...
                @if($review->product)
                  <span class="inline-block text-[11px] font-medium text-stone-500 truncate max-w-[160px] bg-stone-50 px-2 py-0.5 rounded-md border border-stone-150 shrink-0">
                    {{ $review->product->name }}
                  </span>
                @endif
              </div>
