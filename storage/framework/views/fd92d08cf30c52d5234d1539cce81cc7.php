...
<aside id="sidebar" class="fixed lg:sticky lg:top-0 lg:self-start inset-y-0 left-0 z-50 w-64 max-w-[88vw] h-dvh bg-white border-r border-gray-200 flex flex-col -translate-x-full lg:translate-x-0 transition-transform duration-200">
  <div class="h-16 flex items-center gap-2 px-5 sm:px-6 border-b border-gray-200 shrink-0 relative">
    <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center gap-2 min-w-0 pr-10" aria-label="<?php echo e($site); ?>">
      <img src="<?php echo e(logo_url()); ?>" alt="" class="h-9 w-9 rounded-lg object-contain shrink-0" width="36" height="36" />
      <span class="font-bold text-lg truncate"><?php echo e($site); ?></span>
    </a>
  ...
  <nav class="sidebar-nav flex-1 overflow-y-auto py-4 px-3 text-sm min-h-0">
    <?php $__currentLoopData = $nav; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group => $items): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2 mt-5 first:mt-0"><?php echo e($group); ?></p>
      <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $on = request()->routeIs($item['pattern']); ?>
        <a href="<?php echo e(route($item['route'])); ?>" class="admin-nav-link flex items-center gap-3 px-3 py-2.5 rounded-lg mb-1 <?php echo e($on ? 'active bg-primary/10 text-primary font-medium' : 'text-gray-600 hover:bg-gray-100'); ?>">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><?php echo $item['icon']; ?></svg>
          <span><?php echo e($item['label']); ?></span>
          <?php if(!empty($item['badge']) && $item['badge'] > 0): ?>
            <span class="ml-auto bg-amber-500 text-white text-xs font-semibold px-2 py-0.5 rounded-full"><?php echo e($item['badge']); ?></span>
          <?php endif; ?>
        </a>
  ...
  <div class="border-t border-gray-200 p-4 flex items-center gap-3 shrink-0">
    <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($adminName)); ?>&background=2540e0&color=fff" class="h-9 w-9 rounded-full shrink-0" alt="">
    <div class="min-w-0 flex-1">
      <p class="text-sm font-medium truncate"><?php echo e($adminName); ?></p>
      <p class="text-xs text-gray-400 truncate"><?php echo e($adminEmail); ?></p>
    </div>
...
<?php /**PATH /Users/mohammadshamimhossain/Desktop/appFinal/resources/views/admin/partials/sidebar.blade.php ENDPATH**/ ?>